    ...
    tabsetPanel(
      tabPanel("About",
               p("This apps uses the data collected about NFL players 
                throughtout the league from the website", strong("Kaggle"), "which ranges form 
                1920 to 2016 \n"),
               p("The weights are measured in lbs and the height are in inches \n"),
               p("The dataset contains", em(nrow(Nfl)), "observations and", em(ncol(Nfl)), 
                 " variables \n"),
...
server <- function(input, output) {
  ...
  output$Info2 <- renderText({
    if(nrow(College_data()) > 0){
      ...
      paste("Please select a college/colleges")
    }
  })
